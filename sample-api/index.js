...
app.post('/api/login', (req, res) => {
  const user = db.get('users').find({ username: req.body.username, password: req.body.password }).value()
...
modules.forEach(moduleName => {
  ...
  app.get(`/api/${moduleName}/:id`, (req, res) => {
    const data = db.get(`${moduleName}`).filter(item => item.id === req.params.id);
    return res.json(data);
});
